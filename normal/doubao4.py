...
class LicensePlateRecognizer:
    ...
    def segment_chars(self, plate_img):
        """终极优化：先校正车牌+过滤背景，再分割字符"""
        if plate_img is None or plate_img.size == 0:
            return []

        # ===================== 第一步：车牌背景过滤+形状校正 =====================
        # 1. 只保留车牌的白色背景+黑色字符（过滤其他颜色）
        ...
        # 合并掩码：只保留白色背景+黑色字符（彻底过滤背景）
        mask_plate = cv2.bitwise_or(mask_white, mask_black)
        plate_clean = cv2.bitwise_and(plate_img, plate_img, mask=mask_plate)

        # 2. 灰度化+自适应二值化（只针对车牌区域，无背景干扰）
        gray = cv2.cvtColor(plate_clean, cv2.COLOR_BGR2GRAY)
        # 自适应二值化：比OTSU更适合有轻微阴影的车牌
        ...
        # 3. 形态学清理：去掉小噪点，连接字符断裂
        kernel_denoise = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
        binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel_denoise)  # 开运算去噪
        kernel_connect = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
        binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel_connect)  # 闭运算连接字符

        # ===================== 第二步：精准裁剪字符区域 =====================
        h, w = binary.shape
        # 只保留字符核心区：上下裁20%，左右裁8%（彻底去掉牌照框）
        binary_cropped = binary[
            int(h * 0.2):int(h * 0.8),
            int(w * 0.08):int(w * 0.92)
        ]
        h_crop, w_crop = binary_cropped.shape

        # 调试：显示无背景的二值化图（此时只有字符，无任何干扰）
        if self.debug:
            cv2.imshow("7. Plate Binary (Clean)", binary_cropped)

        # ===================== 第三步：字符轮廓筛选（精准匹配电动车牌字符） =====================
        contours, _ = cv2.findContours(binary_cropped, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        char_imgs = []

        # 电动车牌字符的固定特征（死标准，无例外）
        min_char_height = h_crop * 0.7  # 字符高度至少占裁剪区70%
        max_char_height = h_crop * 0.95  # 字符高度最多占裁剪区95%
        min_char_width = w_crop * 0.08  # 单个字符宽度至少占8%
        max_char_width = w_crop * 0.25  # 单个字符宽度最多占25%

        for cnt in contours:
            x, y, w_char, h_char = cv2.boundingRect(cnt)
            char_area = cv2.contourArea(cnt)

            # 严格筛选：只保留符合电动车牌字符特征的轮廓
            if (min_char_height < h_char < max_char_height and
                    min_char_width < w_char < max_char_width and
                    char_area > 30):  # 排除极小噪点

                # 提取字符并归一化尺寸（方便后续识别）
                char_roi = binary_cropped[y:y + h_char, x:x + w_char]
                char_roi_resized = cv2.resize(char_roi, (30, 60))  # 固定字符尺寸
                char_imgs.append((x, char_roi_resized))

        # 按x坐标排序（保证字符顺序正确）
        char_imgs.sort(key=lambda x: x[0])

        # 调试：显示最终分割的字符（纯字符，无干扰）
        if self.debug and char_imgs:
            char_show = np.hstack([c[1] for c in char_imgs])
