...
class LicensePlateRecognizer:
    ...
    def check_char_texture(self, roi):
        ...
        # 2. 计算纹理密度：黑白像素交替的比例（字符的核心特征）
        # 水平方向像素变化次数（高频变化=有字符）
        h, w = binary.shape
        texture_score = 0
        for row in range(h):
            pixel_changes = np.sum(np.abs(np.diff(binary[row, :]))) / 255
            texture_score += pixel_changes

        # 归一化分数（0-1，越高表示字符纹理越丰富）
        texture_score = texture_score / (h * w)
    ...
    def locate_plate_dual_strategy(self, img_original, img_morph):
        ...
        # 第二步：抗干扰过滤（核心！只保留有字符纹理的候选框）
        valid_candidates = []
        for (x, y, w, h, area, box) in raw_candidates:
            # 提取候选框ROI
            roi = img_original[y:y + h, x:x + w]
            # 计算字符纹理分数（阈值>0.05表示有字符）
            texture_score = self.check_char_texture(roi)
            if texture_score > 0.05:
                valid_candidates.append((x, y, w, h, area, box, texture_score))
                # 调试：标注纹理分数
                if self.debug:
                    cv2.putText(img_contours, f"{texture_score:.2f}", (x, y - 5),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)

        # 第三步：颜色区域补充（仅当有效候选框为空时）
        if not valid_candidates:
            print("⚠️  无有效纹理候选框，启用颜色区域补充...")
        ...
        # 第四步：排序（优先选纹理分数高+面积大的）
        if not valid_candidates:
            print("❌ 双策略定位均失败（无有效字符纹理）")
            return None

        # 排序权重：纹理分数(0.7) + 面积(0.3)
        valid_candidates.sort(key=lambda x: (x[6] * 0.7 + x[4] / 10000 * 0.3), reverse=True)
        best = valid_candidates[0]
        x, y, w, h, _, box, _ = best
        ...
        cv2.putText(img_plate_box, f"Texture: {best[6]:.2f}", (x_start, y_start - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        if self.debug:
            cv2.imshow("5. Final Plate Box (Anti-Noise)", img_plate_box)
