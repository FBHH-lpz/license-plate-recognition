...
class LicensePlateRecognizer:
    ...
    def recognize_smart(self, top_imgs, bot_imgs):
        ...
        # 统计指标
        total_chars_found = 0
        suspicious_objects = 0  # 可能是遮挡物（切出来了但匹配分很低）

        # 阈值设置
        MIN_SCORE = 0.45  # 认为是有效字符的最低分
        OCCLUSION_SCORE = 0.35  # 低于这个分，认为是遮挡物/噪点
        ...
        for i, img in enumerate(top_imgs):
            ...
            if best_score > MIN_SCORE:
                top_text += best_char
                total_chars_found += 1
                print(f"  上层[{i}]: '{best_char}' (分:{best_score:.2f}) -> ✅ 正常")
            elif best_score < OCCLUSION_SCORE:
                suspicious_objects += 1
                top_text += "?"
                print(f"  上层[{i}]: 未知物体 (分:{best_score:.2f}) -> ⚠️ 疑似遮挡/污渍")
            else:
                top_text += "?"  # 分数尴尬区间，可能是模糊

        # 城市修正
        city_map = {'广': '广州', '佛': '佛山', '山': '佛山', '东': '东莞', '莞': '东莞'}
        final_top = top_text
        for k, v in city_map.items():
            if k in top_text: final_top = v; break
        ...
        print(f"  有效字符数: {total_chars_found}")
        print(f"  可疑遮挡物: {suspicious_objects}")

        status = "正常"

        # 判定规则 1: 有明显的异物 (如贴纸、泥块)
        if suspicious_objects >= 1:
            status = "【警告】车牌存在局部遮挡/污渍"

        # 判定规则 2: 有效字符太少 (被大面积挡住或缺失)
        # 正常两轮车牌通常至少有 5-6 个字符 (2汉字+数字)
        elif total_chars_found < 4:
...
if __name__ == "__main__":
    img_path = "images/cover/4.jpg"  # 换成你的测试图片
