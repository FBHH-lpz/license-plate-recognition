...
def generate_templates():
    ...
    # 🔵 PART 2: 汉字 (从文件夹加载)
    # ---------------------------------------
    template_dir = "templates_bold"  # 对应第一步生成的文件夹名
...
class LicensePlateRecognizer:
    ...
    # ---------------------------------------------------------
        # ---------------------------------------------------------
        # 🟢 模块 C：字符分割 (Debug特别版：聚焦中间区域 + 面积优选)
        # ---------------------------------------------------------
        # ---------------------------------------------------------
        # 🟢 模块 C: 字符分割 (V1 原版逻辑 + 调试可视化)
        # ---------------------------------------------------------
        # ---------------------------------------------------------
        # 🟢 模块 C: 字符分割 (V1 逻辑 + 防粘连剃头 + 蓝色全显模式)
        # ---------------------------------------------------------
    def segment_chars(self, plate_img):
        if plate_img is None or plate_img.size == 0: return [], []

        # 1. 预处理 (去边框 - 加大力度)
        # 原来是 3，现在改成 5，尽量把白边切干净
        ...
        # --- 【核心修改】剃头操作 (Anti-Glue) ---
        # 很多时候汉字会和上面的螺丝或边框粘连。
        # 我们强行把顶部 12% 的区域涂黑 (变成背景)，切断联系。
        safe_zone = int(split_y * 0.35)
        ...
        # 调试容器
        debug_raw_contours = []  # 蓝色：所有找到的原始轮廓
        top_candidates = []  # 绿色/红色：进入决赛圈的

        for cnt in cnts_top:
            x, y, w_c, h_c = cv2.boundingRect(cnt)
            cx = x + w_c / 2

            # 存入蓝色列表 (只要是个轮廓就存，看看"州"变成了什么鬼样子)
            debug_raw_contours.append((x, y, w_c, h_c))

            # --- 原始筛选逻辑 ---
            if cx > w * 0.85 or cx < w * 0.15: continue

            # 放宽一点高宽比，防止"州"这种稍微扁一点的字被杀掉
    ...
    def recognize_smart(self, top_imgs, bot_imgs):
        ...
        for idx, img in enumerate(top_imgs):
            ...
            # 【V1 核心】：
            # 如果是灰度图，进行二值化处理，模拟最终匹配时的状态
            if len(img.shape) == 2:
                # 这里使用的是你原本逻辑中的 127 阈值
...
if __name__ == "__main__":
    img_path = "images/distraction/3.jpg"
